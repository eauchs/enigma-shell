...
// Assurez-vous que ces constantes sont définies si fetchLlmEnigma les utilise
const LOCAL_LLM_ENDPOINT_ENIGMA = "http://localhost:1234/v1/chat/completions"; // REMPLACEZ CECI
const LOCAL_LLM_MODEL_NAME_ENIGMA = "qwen/qwen3-4b"; // REMPLACEZ CECI
...
const App: React.FC = () => {
  ...
  const handleEmulatorReady = useCallback(() => {
    console.log("App.tsx: handleEmulatorReady A ÉTÉ APPELÉE ! Mise à jour de isEnigmaOSReady à true.");
    setIsEnigmaOSReady(true);
    setEnigmaOSOutputHistory(prev => prev + "INFO: enigma-os (Alpine Linux) est prêt et attend les commandes.\n");
  }, []);

  const handleEnigmaOSOutput = useCallback((outputChunk: string) => {
    // console.log("App.tsx: handleEnigmaOSOutput reçu:", JSON.stringify(outputChunk)); // Peut être très verbeux
    setEnigmaOSOutputHistory(prev => prev + outputChunk);
  }, []);

  const handleCommandSentToOS = useCallback(() => {
    console.log("App.tsx: handleCommandSentToOS - commande réinitialisée.");
  ...
  const fetchLlmEnigma = useCallback(async (): Promise<MathProblem | null> => {
    ...
    console.log("App.tsx: fetchLlmEnigma - Début");
    try {
...
.
      ...
      if (!response.ok) {
        ...
        console.error("App.tsx: fetchLlmEnigma - Erreur de requête:", errorData);
        throw new Error(errorData.error?.message || errorData.message || `La requête pour l'énigme a échoué: ${response.status}`);
      }

      const data = await response.json();
      console.log("App.tsx: fetchLlmEnigma - Réponse LLM brute:", data);
      let enigmaContent = data.choices?.[0]?.message?.content;

      if (!enigmaContent) {
        console.error("App.tsx: fetchLlmEnigma - Pas de contenu dans la réponse LLM.");
        throw new Error("Le LLM n'a pas retourné de contenu pour l'énigme.");
      }
      console.log("App.tsx: fetchLlmEnigma - Contenu extrait:", enigmaContent);

      const jsonMatch = enigmaContent.match(/```json\n([\s\S]*?)\n```|({[\s\S]*})/);
      if (jsonMatch) {
        enigmaContent = jsonMatch[1] || jsonMatch[2];
        console.log("App.tsx: fetchLlmEnigma - Contenu après extraction du bloc JSON:", enigmaContent);
      }
      
      const parsedEnigma = JSON.parse(enigmaContent);
      console.log("App.tsx: fetchLlmEnigma - Énigme parsée:", parsedEnigma);

      if (typeof parsedEnigma.question === 'string' && typeof parsedEnigma.answer === 'number') {
        console.log("App.tsx: fetchLlmEnigma - Succès, énigme valide.");
        return { question: parsedEnigma.question, answer: parsedEnigma.answer };
      } else {
        console.error("App.tsx: fetchLlmEnigma - Format d'énigme invalide:", parsedEnigma);
        throw new Error("La réponse du LLM pour l'énigme n'est pas au format attendu.");
      }

    } catch (error) {
      console.error("App.tsx: fetchLlmEnigma - Échec global:", error);
      setAccessMessage(`Erreur de génération du challenge IA: ${error instanceof Error ? error.message : "Erreur inconnue"}. Retour au challenge standard.`);
      await new Promise(resolve => setTimeout(resolve, 1500));
      return null;
    } finally {
      setIsLoadingEnigma(false);
      console.log("App.tsx: fetchLlmEnigma - Fin");
    }
  }, []);
  
  const generateFallbackMathProblem = useCallback((): MathProblem => {
    console.log("App.tsx: generateFallbackMathProblem appelée.");
    const operations = ['+', '-', '*'];
  ...
  const handleUnlockSuccess = useCallback(async () => {
    console.log("App.tsx: handleUnlockSuccess - Début");
    ...
    if (!problem) {
        console.log("App.tsx: handleUnlockSuccess - Échec de fetchLlmEnigma, utilisation du fallback.");
  ...
  // Log pour suivre l'état de isEnigmaOSReady
  useEffect(() => {
    console.log("App.tsx: Valeur de isEnigmaOSReady a changé:", isEnigmaOSReady);
  }, [isEnigmaOSReady]);
      ...
      {appPhase === AppPhase.SHELL_ACTIVE && (
        <div style={{ display: 'flex', flexDirection: 'row', width: '95vw', maxWidth: '1800px', height: 'calc(100vh - 150px)', gap: '15px' }}>
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
            <Shell
              onCommandForEnigmaOS={setCommandForEnigmaOS}
